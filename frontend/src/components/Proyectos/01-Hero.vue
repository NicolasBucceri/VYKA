...
<template>
  <section
    class="hero"
    :aria-label="t('projectDetail.heroAria')"
    v-if="proyecto"
  >
    <!-- Collage -->
    <div class="top" v-if="imgsOk">
      <HeroCollage
        ...
        :veilOpacity="proyecto.heroVisual?.veil ?? 0.48"
      />
    </div>
    ...
    <div class="content">
      ...
      <div class="actions" v-if="proyecto.siteUrl">
        <a
          class="btn primary"
          :href="proyecto.siteUrl"
          target="_blank"
          rel="noopener"
        >
          {{ proyecto.ctaLabel || t("projectDetail.viewProject") }}
  ...
  >
    <div class="content">
      <p class="kicker">{{ t("projectDetail.loadingKicker") }}</p>

      <h1 class="title">
        <span class="tWhite">{{ t("projectDetail.loadingTitle") }}</span>
      </h1>
    </div>
  </section>
  ...
  >
    <div class="content">
      <p class="kicker">{{ t("projectDetail.notFoundKicker") }}</p>

      <h1 class="title">
        <span class="tWhite">{{ t("projectDetail.notFoundTitle") }}</span>
      </h1>
...
const imgsOk = computed(() => {
  return Array.isArray(props.proyecto?.heroImgs) && props.proyecto.heroImgs.length === 3;
});
</script>
